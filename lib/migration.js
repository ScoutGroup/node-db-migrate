...
Migration.prototype._up = function() {
  if(this.path){
    ...
    return this.migrator.up.apply(this, arguments);
  }
};
...
Migration.prototype._down = function() {
  if(this.path){
    ...
    return this.migrator.down.apply(this, arguments);
  }
};
...
Migration.loadFrom = function(hash, callback) {
  if(!hash){
    return callback(null, []);
  }
  var names = Object.keys(hash).sort();
  var migrations = names.sort().map(function(name) {
    return new Migration(name, hash[name]);
  });
  callback(null, migrations);
};
